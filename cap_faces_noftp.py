import cv2
import os
import imutils
from tqdm import tqdm

personName = 'Gia'
dataPath = 'C:/Users/Pc/Documents/HandgunDataset/Reconocimiento Facial/data'
personPath = dataPath + '/' + personName

# Create local folder if it does not exist
if not os.path.exists(personPath):
    print('Carpeta creada:', personPath)
    os.makedirs(personPath)

cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
faceClassif = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
count = 0

# Total images to process
total_images = 100

# Bar
progress_bar_creation = tqdm(total=total_images, desc='Creando imágenes localmente')

while True:
    ret, frame = cap.read()
    if ret == False:
        break
    frame = imutils.resize(frame, width=640)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    auxFrame = frame.copy()

    faces = faceClassif.detectMultiScale(gray, 1.3, 5)

    for (x, y, w, h) in faces:
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        rostro = auxFrame[y:y+h, x:x+w]
        rostro = cv2.resize(rostro, (150, 150), interpolation=cv2.INTER_CUBIC)

        # Save
        local_file_path = os.path.join(personPath, f'{personName}_rostro_{count}.jpg')
        cv2.imwrite(local_file_path, rostro)

        # Increase and update
        count += 1
        progress_bar_creation.update(1)

    if count >= total_images:
        progress_bar_creation.close()
        progress_bar_creation.refresh()

    cv2.imshow('frame', frame)

    k = cv2.waitKey(1)
    if k == 27 or count >= total_images:
        break

# Close the progress bar
progress_bar_creation.close()

cap.release()
cv2.destroyAllWindows()
